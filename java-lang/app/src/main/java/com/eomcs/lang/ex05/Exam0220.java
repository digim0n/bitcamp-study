...
public class Exam0220 {
  public static void main(String[] args) {
    ...
    //  정규화 ( 소수점 이하의 수를 이진수로 바꾸는 ) 과정에서 2진 정수로 딱 떨어지지 않는경우가 있다.
    // 즉, 극한의 미세소수점이 붙을수있다.
    // 이럴경우, - CPU나 OS, JVM의 문제가 아니다.
    // - IEEE 754 명세에 따라 부동소수점을 처리하는 모든 
    //   컴퓨터에서 발생하는 문제이다.
    // 이런 부동소수점을 계산할때 기대하는 값과 다른값이 나올수있다. 위에서 double d1+d2를 기대할때 988.7어쩌고 나오길 기대하는데 다르다고
    // 또한 연산한 결과를 메모리에 담을때도 정규화과정에서
    // 극한의 미세 소수점이 붙을수있다. 출력할때 극한의 소수값이 뒤에 붙는다 쓰레기같은것들이... 그래서 에러뜬당 어떻게 처리 해야 되나........ㅠ밑에있음 읽어라
    ...
    // 어떻게 하면 되냐 -------에 대한답: 소수점 뒤에 붙은 극소수의 값을 걍 무시하면 된다. ^^
    // => JVM이 자동으로 처리하지 않는다.
